    ...
    <button onclick="topFunction()" id="myBtn" title="Retourner en haut de page"><i class="fa fa-arrow-up" aria-hidden="true"></i></button>
        <?php
            ...
            if($_SESSION['email'] != ''){
                ...
                $resultat = $cnn->query($requete) or die(print_r($bdd->errorInfo()));
                while($row = $resultat->fetch()){
					#On affiche les informations de l'utilisateur et des boutons d'options
                    echo "<center><div class='container'><center><h2>Informations de mon compte</h2></center><i class='fa-solid fa-tag'></i> Pseudo : {$row['pseudoMemb']}<br><br><i class='fa-solid fa-at'></i> Email : {$row['idMemb']}<br><br><i class='fa-solid fa-clock'></i> Date d'inscription : {$row['dateIns']}<br><a class='button cl1' href='edit'>Modifier votre compte</a><br><a class='button cl2' href='../script/delete_account.php'>Supprimer votre compte</a><br><br></div></center>";
                }                    
            }
            else{
				#Si l'utilisateur n'est pas connecté
                echo"<div class='container'><center><p>Vous n'êtes pas connecté</p><a class='button cl1' href='../'>Retourner à l'accueil</a></center><br></div>";
            }
        ?>
    </body>
</html>
