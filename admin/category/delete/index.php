...
<html>
  ...
  <body>
    ...
    <br>
    <div class="container">
    <center><h1>Suppression d'une catégorie</h1></center><br>
    <?php
      ...
      if($_SESSION['email'] != ''){
          ...
          if($row['typeMemb'] == '0'){                  
            ...
            while($row1 = $resultat1->fetch()){
              if($row1['count(nomCat)'] != 0) {
                $requete2 = "SELECT nomCat, iconCat, idCat FROM categorie";
                $resultat2 = $cnn->query($requete2) or die(print_r($bdd->errorInfo()));
                #On affiche les catégories à supprimer
                while($row2 = $resultat2->fetch()){
                  echo"<center><a href='../../../script/delete_category.php?category={$row2['idCat']}' style='text-decoration: none; color: white;'><p class='category cl1' style='width: 50%; transition: 0.4s;'><i class='{$row2['iconCat']}'></i> {$row2['nomCat']}</p></a></center><br>";
                }
              }
              #Si il n'y a pas de catégories à supprimer
              else {
                echo "<center><p>Il n'y a pas de catégories à supprimer</p></center>";
              }
            }
          }
          #Si l'utilisateur n'a pas les permissions
          else {
            echo"<center><p>Vous n'avez pas les permissions pour supprimer une catégorie</p></center>";
          }
        }
      }
      else {
        ...
        echo"<center><p>Vous n'êtes pas connecté</p></center>";
		  ...
		  echo"<center><a href='../../../' class='button cl1'>Retour</a></center><br></div>";
    ?>
  </body>
</html>
