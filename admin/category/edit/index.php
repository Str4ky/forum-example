...
<html>
  ...
  <body>
    ...
    <br>
    <div class="container">
    <center><h1>Modification d'une catégorie</h1></center><br>
    <?php
		  #Si l'utilisateur est connecté
      if($_SESSION['email'] != ''){
        ...
        while($row = $resultat->fetch()){
          ...
          if($row['typeMemb'] == '0'){
            $requete1 = "SELECT count(nomCat), idCat FROM categorie";
            $resultat1 = $cnn->query($requete1) or die(print_r($bdd->errorInfo()));
            while($row1 = $resultat1->fetch()){
              if($row1['count(nomCat)'] != 0) {
                echo "<div class='text'>
                <center><form method='post' action='../../../script/edit_category.php'>
                <label for='nom'>Catégorie</label><br><br>
                <select name='category' id='category' onchange='change_name(this);'>";
                $requete2 = "SELECT idCat, nomCat, iconCat FROM categorie";
                ...
                <label for='nom' class='title2'>Nom de la catégorie</label><br><br>
              ...
              echo"<center><p>Il n'y a pas de catégories à modifier</p></center>";
              }
            }
          }
...
<!--  Le script de récupération du nom de la catégorie, de l'icône, du bouton de retour en haut de page et de connexion -->
<script>
  function change_name() {
    option = document.getElementById('category');
