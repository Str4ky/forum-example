...
<html>
  ...
  <body>
    ...
    <br>
    <div class="container">
    <center><h1>Suppression d'une rubrique</h1></center><br>
    <?php
			#Si l'utilisateur est connecté
      if($_SESSION['email'] != ''){
        ...
        while($row = $resultat->fetch()){
          ...
          if($row['typeMemb'] == '0'){     
            ...
            $resultat1 = $cnn->query($requete1) or die(print_r($bdd->errorInfo()));
            while($row1 = $resultat1->fetch()){
              if($row1['count(nomRub)'] != 0) {             
                $requete2 = "SELECT nomRub, idRub, descRub FROM rubrique";
                $resultat2 = $cnn->query($requete2) or die(print_r($bdd->errorInfo()));
                while($row2 = $resultat2->fetch()){
                  echo"<center><u><a href='../../../script/delete_rubrique.php?rubrique={$row2['idRub']}' style='text-decoration: none; color: white;'><p class='rubrique cl1'>{$row2['nomRub']}</u><br><br>{$row2['descRub']}</p></a></center><br>";
                }
              }
              #Si il n'y a pas de rubrique
              else {
              echo"<center><p>Il n'y a pas de rubrique à supprimer</p></center>";
              }
            }
          }
          #Si l'utilisateur n'a pas les permissions
          else {
            echo"<center><p>Vous n'avez pas les permissions pour supprimer une rubrique</p></center>";
          }
        }
      }
      else {
