    ...
    <?php
      if(isset($_GET['rubrique'])) {
        ...
        while($row = $resultat->fetch()){
          ...
          if($row['count(titreArt)'] == 0) {
            echo "<center><p>Il n'y a pas encore d'article</p></center>";
          }
        }
        ...
        while($row1 = $resultat1->fetch()){
          ...
          while($row2 = $resultat2->fetch()){
            echo "<center><a href='../article?article={$row1['idArt']}&amp;rubrique={$_GET['rubrique']}' class='article cl1' style='text-decoration: none; color: white; font-size: 22px;'>Posté par ";
            ...
            while($row3 = $resultat3->fetch()){
              ...
              echo "<br><br>{$row1['titreArt']}</a><br><br>";
            }
          }
        }
      }
      ...
      if($_SESSION['email'] != ''){
        ...
        while($row4 = $resultat4->fetch()){
          #On vérifie le type de membre (admin ou modo) et on affiche le bouton de suppression d'articles
          if($row4['typeMemb'] == '0' OR $row4['typeMemb'] == '1'){
            echo"<a href='delete?rubrique={$_GET['rubrique']}' class='button cl2' style='text-decoration: none; text-align: center;'>Supprimer un article <i class='fas fa-trash'></i></a></center>";
          }
        }
      }
      echo"<center><a href='../' class='button cl1'>Retour à l'accueil</a><br><br></div></center>";
    ?>
  </body>
</html>
